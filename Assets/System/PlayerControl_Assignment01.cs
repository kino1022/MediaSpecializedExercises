...
namespace System {
    /// <summary>
    /// 初回課題用に改変したPlayerControl
    /// </summary>
    public class PlayerControlAssignment01 : SerializedMonoBehaviour {
        //プレイヤーキャラのアニメーター登録
        ...
        //変更開始：空中ジャンプ力の調整用変数を追加
        [SerializeField]
        [LabelText("空中時のジャンプ倍率")]
        ...
        void FixedUpdate() {
            ...
            //変更
            //float * float * float * FVectorに変更
            rb.AddForce(moveSpeed * speedLimit * sprintSpeedForce * movement, ForceMode.Force);
            //変更終了
            ...
            if (doJump) {
                ...
                if (playable) { 
                    //変更終了
                    //変更開始：空中ジャンプ力の調整を追加
                    var force = isGrounded ? initialJumpForce : initialJumpForce * _onAirRatio;
        ...
        bool checkGrounded() {
            ...
            //接触情報はhitに格納
            return Physics.SphereCast(transform.position + groundCheckOffsetY * Vector3.up, groundCheckRadius,
                Vector3.down, out hit, groundCheckDistance, LayerMask.GetMask("Default"),
        ...
        void setAnimation() {
            ...
            if (Mathf.Abs(rbVelocity) > 3.0f && rbVelocity * inputMoveAxis.x < 0)
